...
class Dawn:
    ...
    def welcome(self):
        print(
            f"""
        {Fore.GREEN + Style.BRIGHT}Dawn - BOT |  {Fore.YELLOW + Style.BRIGHT}TOOL SHARED BY FORESTARMY (https://t.me/forestarmy)
            """
        )
        #reuse this script we don't have problem but don't claim youself to be the owner
    ...
    async def question(self):
        while True:
            try:
                print(f"{Fore.GREEN + Style.BRIGHT}F O R E S T A R M Y || D A W N V A L I D A T O R || S A T Y A V I R{Style.RESET_ALL}")
                print("1. Run With Auto Proxy | RANDOM PROXY")
                print("2. Run With Manual Proxy | USING PERSONAL PROXY")
                print("3. Run Without Proxy | USING IP")
